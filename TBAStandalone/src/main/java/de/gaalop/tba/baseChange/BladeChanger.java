 ...
 * 
 * TODO
 * vermutlich brauche ich das für die Tabelle gar nicht mehr, aber vielleicht später wieder?
 * 
...
public class BladeChanger {
     ...
     */
    private static List<BladeStr> cartesian(List<List<BladeStr>> list) {
        if (list.size() == 1)  //anchor, easy case
            return list.getFirst();
        else {
            //complex case
            List<BladeStr> result = new LinkedList<>();

            // recursive call on sublist case
            List<List<BladeStr>> subList = new LinkedList<>(list);
            List<BladeStr> thisElement = subList.removeFirst();
            List<BladeStr> subResult = cartesian(subList);
            
            // merge
            for (BladeStr comp: thisElement) {
                for (BladeStr subResultComp: subResult) {
                    //concatenate arrays
                    List<String> both = new LinkedList<>();
                    Collections.addAll(both, comp.getBaseVectors());
                    Collections.addAll(both, subResultComp.getBaseVectors());
                    
                    BladeStr b = new BladeStr(comp.getPrefactor()*subResultComp.getPrefactor(), both.toArray(String[]::new));
     ...
     */
    public List<PrefactoredBladeIndex> transform(int fromIndex) {
        if (fromIndex == 0) {
            List<PrefactoredBladeIndex> result = new LinkedList<>();
        ...
        List<List<BladeStr>> bladeExpressionTransformed = new LinkedList<>();
        ...
        List<BladeStr> cartesianResult = cartesian(bladeExpressionTransformed);
        ...
        // Transform
        Map<Integer, PrefactoredBladeIndex> result = new HashMap<>();
        for (BladeStr b: cartesianResult) {
