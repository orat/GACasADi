...
import static de.orat.math.gacasadi.impl.PgaMvValue.create;
...
public class PgaMvValue extends DelegatingGaMvValue implements IMultivectorValue<PgaMvValue, GaMvExpr>, IGetSparsityCasadi {
     ...
     */
    @Deprecated
    private PgaMvValue(GaMvExpr sym, ComposableImmutableBinaryTree<PgaMvValue> inputs) {
        super(sym);
        this.inputs = inputs;
    }
    ...
    @Deprecated
    @Override
    protected PgaMvValue create(GaMvExpr sym) {
        // Call permitted here.
    ...
    @Deprecated
    @Override
    protected PgaMvValue create(GaMvExpr sym, PgaMvValue other) {
    ...
    public static PgaMvValue createFrom(GaMvExpr sym) {
        /*
    ...
    public static IConstants<PgaMvValue> constants2() {
        return GaFactory.instance.constantsValue();
    }
    ...
    // tested
    private static PgaMvValue createIPNSPlane(GeometricObject obj){
        ...
        */
        PgaMvValue attitude = createE3(obj.attitude);
    ...
    private static PgaMvValue createIPNSLine(GeometricObject obj){
        // local blade = weight * ( normal + ( center ^ normal ) * ni ) * i
        PgaMvValue inf = constants2().getBaseVectorInfinity();
        PgaMvValue I3 = constants2().getEuclideanPseudoscalar();
        PgaMvValue nq = createE3(obj.attitude);
        PgaMvValue q = createE3(obj.location[0]);
        return nq.add(q.op(nq).gp(inf)).
                gp(I3).gpWithScalar(obj.getSignedWeight());
    }
    
    
    private static PgaMvValue createIPNSFlatPoint(GeometricObject obj){
        PgaMvValue inf = constants2().getBaseVectorInfinity();
        PgaMvValue I3 = constants2().getEuclideanPseudoscalar();
        PgaMvValue one = constants2().one();
    ...
    private static PgaMvValue createE3(Tuple c){
    ...
    private GeometricObject decomposeFlatPoint(boolean isIPNS){
        PgaMvValue attitude = decomposeAttitudeFlatAsEInf(isIPNS);
     ...
     */
    private static Tuple locationFlatPoint(PgaMvValue m1, boolean isIPNS){
        PgaMvValue m = m1;
        if (isIPNS) m = m1.undual();
        PgaMvValue inf = m.constants2().getBaseVectorInfinity();
        PgaMvValue o = m.constants2().getBaseVectorOrigin();
        PgaMvValue oinf = o.op(inf);
        
        // Dorst2007 drills 14.9.2. nr. 5
        PgaMvValue result = oinf.lc(o.op(m)).div(oinf.lc(m)).negate();
     ...
     * @return attitude 
     */
    private static Tuple attitudeFlatPoint(boolean isIPNS, PgaMvValue m){
        PgaMvValue m1 = m;
        if (isIPNS) m1 = m.dual();
        // corresponding to
        ...
        PgaMvValue inf = m1.constants2().getBaseVectorInfinity();
        PgaMvValue result =  inf.negate().lc(m1);
        //return new CGAAttitudeOPNS(result);
        return extractE3(result);
    }
    
    private GeometricObject decomposeScrewAxisOrLine(boolean isIPNS, PgaMvValue probePoint){
        PgaMvValue attitude = decomposeAttitudeFlatAsEInf(isIPNS);
    ...
    private GeometricObject decomposePlane(boolean isIPNS, PgaMvValue probePoint){
        PgaMvValue attitude = decomposeAttitudeFlatAsEInf(isIPNS); // bivector^einf, grade-3 
    ...
    private Tuple decomposeLocationFlat(boolean isIPNS, PgaMvValue probePoint){
        PgaMvValue m = this;
        if (!isIPNS) m = dual();
       
        PgaMvValue result = probePoint.op(m).div(m); // round point ipns
        
        // extract E3 from normalized dual sphere
        // [Dorst2007] p.409
        PgaMvValue o = constants2().getBaseVectorOrigin();
        PgaMvValue inf = constants2().getBaseVectorInfinity();
        PgaMvValue oinf = o.op(inf);
    ...
    private double squaredSizeOfRound(boolean isIPNS){
        ...
        // verwenden kann
        PgaMvValue inf = constants2().getBaseVectorInfinity();
        PgaMvValue result = gp(gradeInversion()).div((inf.lc(this)).square());  
     ...
     */
    private static double squaredWeight(PgaMvValue attitude, PgaMvValue probePoint){
     ...
     */
    private PgaMvValue attitudeFromTangentAndRoundIPNS(){
        PgaMvValue inf = constants2().getBaseVectorInfinity();
     ...
     * @return location 
     */
    private static Tuple locationFromTangentAndRound(PgaMvValue mm){
        ...
        PgaMvValue inf = mm.constants2().getBaseVectorInfinity();
        PgaMvValue o = mm.constants2().getBaseVectorOrigin();
        PgaMvValue location = mm.negate().div(inf.lc(mm));
    ...
    private GeometricObject decomposeCircleOrOrientedPoint(boolean isIPNS){
        ...
        if (Precision.equals(squaredSize, 0, GeometricObject.eps)){
            ...
            PgaMvValue a = inf.negate().lc(undual()).op(inf);
        ...
        // circle
            ...
            PgaMvValue o = constants2().getBaseVectorOrigin();
            PgaMvValue result = o.op(inf).negate().
    ...
    public double weight(){
        ...
        PgaMvValue inf = constants2().getBaseVectorInfinity();
        PgaMvValue o = constants2().getBaseVectorOrigin();
        PgaMvValue result = o.op(inf).ip(this.op(inf)).norm();
        return extractScalar(result);
    }
    ...
    private GeometricObject decomposeDipole(boolean isIPNS){
        PgaMvValue attitude = attitudeFromTangentAndRoundIPNS(); // ok
        ...
        PgaMvValue center = locationFromTangendAndRound();
     ...
     */
    private PgaMvValue decomposeAttitudeFlatAsEInf(boolean isIPNS){
        PgaMvValue inf = constants2().getBaseVectorInfinity();
        PgaMvValue m = this;
        if (isIPNS) m = this.undual();
     ...
     */
    private PgaMvValue locationFromTangendAndRound(){
        PgaMvValue inf = constants2().getBaseVectorInfinity();
        // corresponds to the errata of the book Dorst2007
        PgaMvValue result = (this.gp(inf).gp(this)).div((inf.ip(this)).square()).gpWithScalar(-0.5d);
     ...
     * @return squared weight > 0
     */
    private static double squaredWeight(PgaMvValue attitude){
     ...
     */
    private static Tuple extractE3FromBivectorInf(PgaMvValue m){
        PgaMvValue o = m.constants2().getBaseVectorOrigin();
        PgaMvValue I3i = m.constants2().getBaseVectorX().op(m.constants2().getBaseVectorY()).
                                             op(m.constants2().getBaseVectorZ()).negate();
        PgaMvValue result = m.gradeSelection(3).rc(o).negate().lc(I3i);
        return extractE3(result);
    }
    private static Tuple extractE3FromVectorInf(PgaMvValue m){
        PgaMvValue o = m.constants2().getBaseVectorOrigin();
        PgaMvValue result = m.gradeSelection(2).rc(o).negate();
        return extractE3(result);
    }
    
    private static Tuple extractE3(PgaMvValue m){
    ...
    private static double extractScalar(PgaMvValue m){
